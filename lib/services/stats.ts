...
export async function getStats(userId: number, year?: number, from?: Date, to?: Date, options: { forceRefresh?: boolean } = {}): Promise<StatsResult> {
    ...
    try {
        ...
        // We can reuse the earlier aggregations or do a quick distinct query
    ...
    if (averageQualityScore > 0) {
        ...
        const PERSONAS = [
            {
                min: 90, options: [
                    { t: "The Certified Fresh 🍅", d: "You trust critics blindly. Have you ever had an original thought, or do you wait for Rotten Tomatoes to tell you what to think?" },
                    { t: "The Film Snob 🧐", d: "Only masterpieces will do. Your library looks like the syllabus for a film studies degree. Boring, but impressive." }
                ]
            },
            {
                min: 80, options: [
                    { t: "The Curator 🏛️", d: "You have taste, we'll give you that. You separate the wheat from the chaff, but you miss out on the charm of a truly terrible action movie." },
                    { t: "The Oscar Bait 🏆", d: "It seems you only watch movies trying to win awards. Do you cry at everything, or are you just pretending?" }
                ]
            },
            {
                min: 70, options: [
                    { t: "The Mainstream Connoisseur 🍿", d: "You like good stuff, but you aren't afraid to dig in the bargain bin occasionally. A healthy balance between art and entertainment." },
                    { t: "The Crowd Pleaser 🤝", d: "Your taste is like a perfect party playlist – no one complains, but no one is surprised either. You are safety personified." }
                ]
            },
            {
                min: 60, options: [
                    { t: "The 'It’s Fine' Enthusiast 🤷", d: "You don't turn it off even if it's lukewarm. Your standard is: 'Does it move? Does it have sound? Okay, let's go.'" },
                    { t: "The Time Killer ⏳", d: "You don't seek quality, you seek distraction. Your average score screams 'I had nothing else to do'." }
                ]
            },
            {
                min: 50, options: [
                    { t: "The Coin Flipper 🪙", d: "Half gold, half trash. Checking your history is like Russian Roulette – you never know if you'll get a classic or a headache." },
                    { t: "The Tolerance Tester 📉", d: "You have an impressive pain threshold. For every good movie you see, you punish yourself with two bad ones. Why?" }
                ]
            },
            {
                min: 40, options: [
                    { t: "The Guilty Pleasure 🫣", d: "We don't judge you. Or well, a little. But we assume you watch all of this 'ironically'? Right? Please say yes." },
                    { t: "The B-Movie Baron 🎬", d: "Budget doesn't seem to be your thing. You prefer movies where the boom mic is in the shot and actors read directly from the script." }
                ]
            },
            {
                min: 20, options: [
                    { t: "The Trash Panda 🦝", d: "You dig in the trash can and find... trash. But you seem to like it. It's a talent to find this much dung." },
            ...
            {
                min: 0, options: [
                    { t: "The Razzies Juror 🗑️", d: "Your top list is the rest of the world's bottom list. Either you're trolling the algorithm, or you hate your eyes." },
